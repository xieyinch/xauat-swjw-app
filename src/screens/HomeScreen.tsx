  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (current) {
        ...
        setTodayLessons(table.lessons.filter((l) => l.dayOfWeek === day && inWeek(l.weekText, Math.max(1, table.currentWeek))));
      } else setTodayLessons([]);
    } catch (e) {
      if ((e as Error).name === 'SessionExpiredError') { onSessionExpired(); return; }
      setError((e as Error).message || '加载失败');
    } finally { setLoading(false); }
  }, [onSessionExpired]);

  useEffect(() => { load(); }, [load]);
  const todayIndex = useMemo(() => { const d = new Date().getDay(); return d === 0 ? 7 : d; }, []);

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.studentCard}>
        <View style={styles.avatar}><Text style={styles.avatarText}>{student?.name?.[0] || user?.name?.[0] || '学'}</Text></View>
        <View style={styles.studentInfo}>
          <Text style={styles.studentName}>{student?.name || user?.name || '同学'}</Text>
          <Text style={styles.studentMeta}>{student?.stdNo || user?.stdNo || '学号未知'}{semesterName ? ` · ${semesterName}` : ''}</Text>
        </View>
        <TouchableOpacity style={styles.logoutBtn} onPress={onLogout} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}><Ionicons name="log-out-outline" size={20} color={colors.textSecondary} /></TouchableOpacity>
      </View>
      <View style={styles.entryGrid}>
        {ENTRIES.map((e) => (
          <TouchableOpacity key={e.key} style={styles.entry} onPress={() => onNavigate(e.key)} activeOpacity={0.7}>
            <View style={[styles.entryIcon, { backgroundColor: `${e.color}1A` }]}><Ionicons name={e.icon as keyof typeof Ionicons.glyphMap} size={26} color={e.color} /></View>
            <Text style={styles.entryLabel}>{e.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
      <View style={styles.section}>
        <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>今日课程</Text><Text style={styles.sectionSub}>{WEEK_LABELS[todayIndex]}</Text></View>
        {loading ? <View style={styles.sectionCenter}><ActivityIndicator color={colors.primary} /></View>
        : error ? <View style={styles.sectionCenter}><Text style={styles.sectionEmpty}>{error}</Text><TouchableOpacity onPress={load}><Text style={styles.retryText}>重试</Text></TouchableOpacity></View>
        : todayLessons.length ? todayLessons.slice().sort((a,b)=>(a.startUnit??99)-(b.startUnit??99)).map((l)=>(
          <View key={l.id} style={styles.lessonCard}><View style={styles.unitBadge}><Text style={styles.unitText}>{l.startUnit ?? '?'}</Text></View><View style={styles.lessonBody}><Text style={styles.lessonName} numberOfLines={2}>{l.nameZh}</Text><Text style={styles.lessonMeta}>{l.timeText}{l.placeText ? ` · ${l.placeText}` : ''}</Text></View></View>
        )) : <View style={styles.sectionCenter}><Ionicons name="happy-outline" size={28} color={colors.textSecondary} /><Text style={styles.sectionEmpty}>今天没课，好好休息</Text></View>}
      </View>
    </ScrollView>
  );
}
